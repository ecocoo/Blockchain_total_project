...
describe("BestYearForGamesVoting", function () {
  ...
  // Тесты функции получения года по индексу
  ...
  describe("Голосование", function () {
    ...
    it("Не должен позволять пользователю голосовать дважды", async function () {
      ...
      await expect(gameVoting.connect(addr1).vote(5)).to.be.revertedWith("You have already voted");
    });

    it("Не должен позволять голосовать за невалидный индекс года", async function () {
      await expect(gameVoting.connect(addr1).vote(11)).to.be.revertedWith("Invalid year index (0-10)");
    });
  ...
  // Тесты ограничения по времени голосования
  describe("Ограничение времени голосования", function () {
    it("Не должен позволять голосовать после окончания времени", async function () {
      ...
      await expect(gameVoting.connect(addr1).vote(3)).to.be.revertedWith("Voting has ended");
    });
  ...
  // Тесты получения информации о голосовании
  ...
  describe("Генерация событий", function () {
    it("Должен генерировать событие Voted при голосовании", async function () {
      await expect(gameVoting.connect(addr1).vote(3)).to.emit(gameVoting, "Voted").withArgs(addr1.address, 3);
    });
  });
